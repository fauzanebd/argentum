...
func (r *renderer) assignWidths(cols []tableColumn, header []string, body [][]string, totals []string) {
	...
	weights := allocate(natural, rigid, contentWidth())
	units := distribute(weights, theme.GridCols, minColUnits)
	for i := range cols {
...
//
// Rigid columns are paid first. If honouring them would leave the flexible
// columns below the readable minimum, nothing is rigid — the table is
// over-subscribed and proportional shrinking is the only honest answer.
// Otherwise the flexible columns divide the remainder in proportion to what
// they asked for.
func allocate(natural []float64, rigid []bool, available float64) []float64 {
	...
	if flexCount == 0 || rigidSum+float64(flexCount)*colWidth(minColUnits) > available {
		return natural
	}
...
// n*minUnits <= sum(out), so there is always a column above the minimum to
// take from while one sits below it.
