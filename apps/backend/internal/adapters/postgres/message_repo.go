...
func (r *MessageRepo) Append(ctx context.Context, m *domain.Message) error {
	const q = `
		INSERT INTO messages (thread_id, role, content, tool_calls, tokens_in, tokens_out, latency_ms, metadata, agent_id)
	...
	return r.db.QueryRowContext(ctx, q,
		...
		// nullOrUUID rather than the bare string: the column is UUID and a
		// user message carries "", which Postgres will not cast (T-N1).
		nullOrUUID(m.AgentID),
...
func (r *MessageRepo) GetForCompany(ctx context.Context, companyID, id string) (*domain.Message, error) {
	...
	// this join and no amount of rewriting it in Go is clearer than the twelve
...
func (r *MessageRepo) ListByThreadRole(ctx context.Context, threadID string, role domain.MessageRole, limit int) ([]*domain.Message, error) {
	...
	q := `SELECT ` + messageColumns + messageFrom + `
		WHERE m.thread_id = $1 AND m.role = $2
...
// messageColumns is the SELECT list every read in this file shares, and
// messageFrom is the FROM clause it must be paired with.
//
// Both are qualified with `m.` and the roster join is folded into the FROM
// rather than added per query (T-N1). The alternative — a second, agent-aware
// column list beside this one — is two lists that drift, and the one that goes
// stale is whichever the next reader does not notice. GetForCompany spells its
// columns out separately because it joins conversation_threads as well and `id`
// is ambiguous across three tables; it reads them in this same order.
//
// LEFT JOIN, not JOIN: agent_id has no foreign key by design (migration 077),
// so it can name an agent that has since been deleted. An inner join would make
// a deleted agent delete its messages from every transcript, which is the exact
// outcome the missing FK exists to prevent.
...
const messageFrom = ` FROM messages m LEFT JOIN agents a ON a.id = m.agent_id`
...
func scanMessage(s interface{ Scan(...any) error }) (*domain.Message, error) {
	...
	if err := s.Scan(&m.ID, &m.ThreadID, &role, &m.Content, &tc,
		&m.TokensIn, &m.TokensOut, &m.LatencyMs, &md, &m.CreatedAt,
		&m.AgentID, &m.AgentName); err != nil {
		return nil, err
	}
...
func (r *MessageRepo) ListPageByThread(ctx context.Context, threadID string, f domain.MessageFilter) ([]*domain.Message, bool, error) {
	...
	args := []any{threadID}
	where := ` WHERE m.thread_id = $1`
	if f.CursorID != "" && !f.CursorTime.IsZero() {
		args = append(args, f.CursorTime, f.CursorID)
		where += ` AND (m.created_at, m.id) > ($` + itoa(len(args)-1) + `, $` + itoa(len(args)) + `::uuid)`
	}
...
// nullOrUUID returns nil for the empty string so a UUID column stores NULL
// rather than refusing the insert. A user message has no agent, and "" is not a
// uuid (T-N1).
func nullOrUUID(s string) interface{} {
	if s == "" {
		return nil
	}
	return s
