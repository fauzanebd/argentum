...
var promptTools = []promptTool{
	...
	{"compute", "compute: Calculate a derived figure — a ratio, margin, growth rate, difference, share or per-unit amount — from numbers an EARLIER tool call in this turn returned. Every result carries a `result_id` (r1, r2, …); pass `expression` in your own names and `inputs` mapping each name to `<result_id>.<column>`, e.g. {\"revenue\": \"r1.total_revenue\"}. An input is ALWAYS a reference, never a number you typed. A column of several rows reduces with sum(), min() or max(). Exact decimal arithmetic: + - * / ( ), comparison, abs, round(x, places), sum, min, max — no loops and no other functions."},
