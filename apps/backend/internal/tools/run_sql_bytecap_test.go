...
func TestMarshalSQLResultUnderTheCapIsUntouched(t *testing.T) {
	res := wideResult(5, 10)
	out := marshalSQLResult("src-1", "postgres", res, 200_000, nil, nil)
...
func TestMarshalSQLResultTrimsWideRowsToFit(t *testing.T) {
	...
	out := marshalSQLResult("src-1", "postgres", res, cap, nil, nil)
	if len(out) > cap {
...
func TestMarshalSQLResultKeepsAsManyRowsAsFit(t *testing.T) {
	...
	marshalSQLResult("src-1", "postgres", res, cap, nil, nil)
	kept := len(res.Rows)
	if kept == 0 || kept == 40 {
...
func TestMarshalSQLResultCapDisabled(t *testing.T) {
	...
	for _, maxBytes := range []int{0, -1} {
		...
		out := marshalSQLResult("src-1", "postgres", res, maxBytes, nil, nil)
		m := decode(t, out)
...
func TestMarshalSQLResultZeroRowsKeepsItsOwnNote(t *testing.T) {
	...
	out := marshalSQLResult("src-1", "postgres", res, 10, nil, nil)
