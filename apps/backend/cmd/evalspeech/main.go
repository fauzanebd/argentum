...
//	GROQ_API_KEY=… OPENAI_API_KEY=… make eval-speech CLIPS=/abs/path/to/recordings
//
...
func main() {
	var (
		...
		providers = flag.String("providers", "groq,openai", "providers to score: groq, openai, custom")
	...
	for _, name := range strings.Split(*providers, ",") {
		...
		default:
			fail(fmt.Errorf("unknown provider %q: groq, openai or custom", name))
		}
		if cfg.APIKey == "" {
		...
		ps = append(ps, evalspeech.Provider{Name: name, Transcriber: tr})
	}
	if len(ps) == 0 {
		fail(fmt.Errorf("no provider to score: set GROQ_API_KEY and/or OPENAI_API_KEY"))
	}
