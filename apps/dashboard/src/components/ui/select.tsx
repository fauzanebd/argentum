...
export const SelectItem = React.forwardRef<
  ...
  React.ComponentPropsWithoutRef<typeof SelectPrimitive.Item> & {
    /**
     * A second line shown under the option in the open list, and *not* in the
     * closed trigger. It sits outside `ItemText` on purpose: Radix clones
     * `ItemText` into `SelectValue` when the item is selected, so anything put
     * in there renders inside the collapsed control too.
     */
  ...
  >
    <span
      className={cn(
        ...
        // Only the two-line variant needs pinning: without a description the
        // item is a single centred row and the static position is already
        // right, which is how every existing call site renders today.
        description && "top-2",
      )}
    >
