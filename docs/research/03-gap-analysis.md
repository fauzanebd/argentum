...
| Q-2 | P0  | **3 of 33 Go packages have tests.** `internal/app` — which contains `ChatRunner`, `ThreadService`, `UsageService`, `ScheduledTaskService`, ~2,900 lines of the highest-risk logic in the system — has none. The dashboard has zero tests.           | `go test ./...`, see `coverage/test-coverage.md` |
| Q-3 | P0  | **CI never runs tests.** The workflow runs `go build` for api and worker only. No `go test`, no `go vet`, no linter, no frontend build check. `cmd/discord` isn't even built.                                                                       | `.github/workflows/ci.yaml`                 |
...
| Q-8 | P2  | `go.mod` declares `go 1.26.1`; CI pins `GO_VERSION: '1.25'`. Works only because `GOTOOLCHAIN=auto` silently downloads 1.26. Fragile and slow.                                                                                                      | `go.mod:3`, `.github/workflows/ci.yaml:26`  |
