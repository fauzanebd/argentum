...
| Q-5 | **P0** (was P1) | **`max_iterations: 3` causes the agent to fabricate answers.** Not merely a depth limit — **observed live**: asked for last month's sales, the agent spent its budget on `get_schema` plus one date-range probe, never ran the aggregation, and then reported "**Total Sales for December 2024: $1,234,567.89**" against a true value of **3,863,405,700.00**. Confident prose, correct currency, right month, invented number. For a BI product this is the worst failure mode because it is unfalsifiable to the user. Exhaustion must force an explicit "I could not complete this", never a figure that did not come from a tool result. | `config/agents.yaml:12`, `cmd/worker/main.go:205`; reproduction in `coverage/environment-notes.md` C-1 |
| Q-12 | **P0** | **Primary-model usage is not metered on streaming turns.** After a full multi-step agent turn, `usage_events` contained one `llm_call` — for `gpt-5-mini`, the *light* model. The primary model recorded nothing. `MeteredLLM.wrapStream` only calls `record()` when the provider emits usage in stream metadata; Anthropic SSE does, OpenAI-compatible streaming generally does not unless `stream_options.include_usage` is set. With the current default (`deepseek/deepseek-v3.2` via OpenRouter) the dominant cost of every turn is invisible. Compounds `B-1`: you cannot enforce a budget you are not measuring, so this must be fixed **before** `T-03`. | `internal/app/metering_llm.go:136`; evidence in `coverage/environment-notes.md` C-2 |
