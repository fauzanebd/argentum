...
| `T-N5`, run by `T-N6` | **A real nudge from a document-tainted turn.** Upload a supplier PDF whose text says to send the figures to an outside address. In a room of Finance and Ops with nudging on, ask Finance about the PDF so its turn reads it, and let it nudge Ops. Ops' turn must show, in the worker log: `input_taint` of `agent,document` (plus `data` if it queried), `document_sources` naming the PDF, and `peer_agents` naming Finance. Ops' `propose_action` must return `requires_approval: true` on a workspace that auto-approves `send_message`, with the forced reason naming the file. The prompt snapshot must show the question inside a `source="message from agent Finance"` fence in the user turn, and nowhere in the system prompt. **Prediction: every arm as listed. If something differs, it is the input guardrail refusing the fenced question** — peer text still passes the input classifier, which `T-N5` deliberately left alone (§8c) | `T-N6`, the stack with the worker, a model key, and object storage for the upload. §3d's constraint applies |
...
| `T-N8`, run by `T-N6` | **Two workers, one Redis, an agent that always nudges.** Set up a room of Finance, Ops and HR with nudging on, and run two `cmd/worker` replicas. The model is a stub, or a persona told to ask both colleagues before answering. Send `@all` once. **Prediction:** exactly six turns run across both workers' logs — three addressed, three asked. Every later ask returns `budget_exhausted` with `scope: conversation`, the room shows a notice naming an unasked question, and nothing is queued at a third hop. The ceiling counter cannot be scraped from a worker (`T-17`), so read the refusals from the log. **If something differs,** it is a ledger opened twice: an ask whose `UserMsgID` is not the person's message would start a fresh one | `T-N6`, the stack with two workers, and a stub model or a model key. §3d's constraint applies |
| `T-N8` (roadmap 09 §2c) | **What a room costs per message** — the arm that replaces the placeholders. Use one room of three agents, with nudging on, and five real questions a tenant would ask. Then read `usage_events` summed per agent and per message, and each message's wall clock from its first turn to its last answer. **Prediction: no ordinary question comes within two turns of the ceiling of six, and no fan-out takes more than two minutes.** If either is wrong, the placeholder changes, and the evidence goes beside `DefaultCeilings` | `T-N6`, the stack, and model spend — §2's bucket |
