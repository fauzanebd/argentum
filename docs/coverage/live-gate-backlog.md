...
| The metric zero path (2026-08-14) | The full set on both models, reading `zero-row-future-quarter` first | **Owed under rule 1.** The change rewrites `query_metric`'s note on every zero and empties `row_count` for a window outside the data, so it changes what reaches the model on the one category that still discriminates. Read the passing `zero_row_trap` cases too: a probe that turned a genuine zero into a hedge would be a regression the aggregate could hide ([`eval-q1.md`](eval-q1.md) *The metric zero path*) |
