...
| `T-23` | One deployed widget on a page, against a tenant whose Settings → Widget carries a **non-default greeting and two starter prompts**: the empty state shows their words and their buttons. Then the transcript route in a network tab, on a thread whose turn ran a query — no SQL, no `source_id`, no tool row | **Owed, added 2026-09-10 with the build.** Both halves are unit-covered and the leak was proven failing before the fix, but the first half is the one that was wrong *in the browser* for a month while every server-side test passed ([`widget.md`](widget.md) §6). It is the cheapest kind of arm there is — no model spend, no second origin, one row in `companies.widget_config` and a page — and the class of defect it catches is the one §1e added a browser to this file for |
