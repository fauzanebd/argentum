...
| `internal/tools` | `run_sql_test.go`, `run_sql_bytecap_test.go`, `run_sql_guard_test.go`, `source_resolve_test.go` | The `run_sql` result notes (`T-16`); the structural check `T-H4` step 3 wired in — eight statements refused (`INSERT`, a second statement behind a `SELECT`, `SELECT … INTO`, `COPY`, `EXEC`, a `DROP` behind a comment, an unbound `{{token}}`, empty) and eight allowed, of which four carry a forbidden keyword inside an identifier, a literal or a comment, because a validator that refuses ordinary analytical SQL costs more than it saves; plus the byte-cap trimming loop — wide rows shrink from the tail, the count matches what was sent, a single oversized row does not become a false "matched zero rows" — and `ResolveSource` with 0/1/many sources, an explicit id, a cross-tenant id, and the empty-company rejection that happens before the repository is touched (`T-02`) |
...
| `internal/report/chart` | `chart_test.go`, `bench_test.go` | The seven chart types, the token palette, determinism, the two no-data states, the series and category caps — and, from `T-R6`, `BenchmarkRenderFullMeasure`: `B/op` for one grouped bar at the PDF's 174mm measure and a deck's 254mm full bleed. It is the only thing in this package drawn at the size a report actually uses; every test beside it is 90mm, a quarter of the pixels, which is why the supersample factor's real cost was invisible here until a worker died of it |
