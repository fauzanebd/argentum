...
| Public `/v1` contract       | 🟡     | `T-A1`: typed error envelope, `X-Request-Id` end to end, `Idempotency-Key` with replay/in-flight/reuse handling, `RateLimit-*` headers, cursor pagination, kill switch, body cap, `api` channel ([`api-foundation.md`](api-foundation.md)). **`T-A2` applied all of it to real routes** — the replay, the mid-flight 409, the changed-body 409 and the request-id → audit chain are live rather than tested-only. Still no published spec: `T-A4` |
