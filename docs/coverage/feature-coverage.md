...
| Public `/v1` contract       | 🟡     | `T-A1`: typed error envelope, `X-Request-Id` end to end, `Idempotency-Key` with replay/in-flight/reuse handling, `RateLimit-*` headers, cursor pagination, kill switch, body cap, `api` channel ([`api-foundation.md`](api-foundation.md)). **`T-A2` applied all of it to real routes** and **`T-A3` closed the last two tested-only items** — the replay, both 409s, the request-id → audit chain and the `channel=api` rollup are all live. `T-A3` also added the one piece the contract was missing: a 5xx that must *not* forget its idempotency key, because the work behind it is still running. Still no published spec: `T-A4` |
