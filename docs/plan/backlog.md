...
| Prune stale feature branches (**12**, re-counted 2026-09-11 — the row said 7) | — | 0.5h | Folded into `T-00b`; otherwise any time. All twelve are fully merged into `origin/main` and nothing is unmerged. **Held for the owner**, because two of them — `origin/pre-monorepo` and `origin/bigref` — read as deliberate markers of a boundary this repo crossed rather than as leftovers, and deleting a remote branch is not something to infer |
...
| ~~Self-host Space Grotesk in the dashboard instead of the Google Fonts CDN~~ | — | ~~0.5h~~ **1h** | **Done 2026-09-11.** And the row's premise was wrong twice: the vendored TTFs have no **600** face, which `font-semibold` uses in 37 places, so reusing them would have shipped every semibold label as bold; and the CDN's Space Grotesk is a **variable** font, so it is one `@font-face` per unicode-range with `font-weight: 300 700`, not one per weight. Three `.woff2` (48 KB, of which a latin visitor loads 22 KB) replace 348 KB of `.ttf` that were never the right format for a browser. `coverage/design-tokens.md` |
| `create_dashboard` fails 7 times for every 2 successes in long turns | Phase 3ae | ? | **Trigger fired 2026-09-11, unclaimed.** 14 errors and 4 blocked against 2 ok, across the 28 production turns that ran six or more tool calls — and concentrated in exactly the turns that run longest, which is `Q-5`'s shape. Sized `?` on purpose: `error_text` has never been read, so nobody knows whether this is one bad argument shape or fourteen different failures. [`coverage/live-gate-backlog.md`](../coverage/live-gate-backlog.md) §3c has the query that would size it |
...
| ~~`DefaultPricing` still labelled "approximates GPT-4o"~~ | B-4 | 0.5h | **Done 2026-09-11**, and the comment was worse than stale: `modelPricing` carries a real `gpt-4o` at $2.50/M in and $10/M out, so the file contradicted itself — `DefaultPricing` is $5/$15. It is not a model's price at all, it is **the rate an unpriced model falls back to**, deliberately above every model this deployment runs (5.3x kimi-k2.6's input rate), and `CREDITS_ENFORCEMENT_ENABLED` issues 402s off it (the kimi-k2.6 incident). The comment now says the fix for a wrong bill is a new `modelPricing` entry, never a change to these two numbers. Comment only; no rate moved |
| Repo owner mismatch (`fauzanebd` module path, `haritsrizkall` GHCR/CI) | — | — | Note only; harmless but confusing to a new contributor |
| A `500` that quotes the database: **101** `StatusInternalServerError, gin.H{"error": err.Error()}` across 20 handler files (`company.go` 18; `slack`, `lark`, `embed_keys`, `discord` 6 each; the rest under 6), counted 2026-09-15 | Found beside `T-W8`'s gate | 1d | **Fixed for feedback only (2026-09-15)**, where `GET /api/messages/x/feedback` put `pq: invalid input syntax for type uuid` in the body ([`coverage/live-gate-backlog.md`](../coverage/live-gate-backlog.md) §7l). Everywhere else a driver's error — which for a dropped connection names the host — reaches the caller. The shape to copy is `feedbackFail`'s: log the error with the company, answer a sentence. Trigger: a security review, or the first body quoting a hostname. Pair it with the malformed-id mapping (`malformedID`), which some repositories apply and some do not |
