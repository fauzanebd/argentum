...
func (r *ThreadRepo) Create(ctx context.Context, t *domain.ConversationThread) error {
	const q = `
		INSERT INTO conversation_threads
			(company_id, channel, phone_number, user_id, title, summary, last_message_at)
		VALUES ($1, $2, NULLIF($3, ''), NULLIF($4, '')::uuid, $5, $6, $7)
		RETURNING id, created_at
	`
	return r.db.QueryRowContext(ctx, q,
		t.CompanyID, string(t.Channel), t.PhoneNumber, t.UserID, t.Title, t.Summary, t.LastMessageAt,
	).Scan(&t.ID, &t.CreatedAt)
}

func (r *ThreadRepo) GetByID(ctx context.Context, id string) (*domain.ConversationThread, error) {
	const q = `
		SELECT id, company_id, channel, COALESCE(phone_number, ''), COALESCE(user_id::text, ''),
...
func scanThreadRow(row rowScanner) (*domain.ConversationThread, error) {
	...
	var channel string
	if err := row.Scan(
		&t.ID, &t.CompanyID, &channel, &t.PhoneNumber, &t.UserID,
